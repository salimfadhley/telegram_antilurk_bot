...
class ShowCommandHandler:
    ...
    async def handle_show_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if not update.message or not context.args:
            if update.message:
                ...
                "❌ Usage: `/antlurk show links|config|reports [limit]`",
                parse_mode='Markdown'
            )
            return
        ...
        if subcommand == "links":
            ...
            await update.message.reply_text(
                "❌ Invalid subcommand. Use: `links`, `config`, or `reports`",
                parse_mode='Markdown'
            )
    ...
    async def _show_links(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if not moderated_channels:
                ...
                for moderated in moderated_channels:
                    ...
                    if moderated.modlog_ref:
                        ...
                        modlog = next(
                            (ch for ch in channels_config.channels if ch.chat_id == moderated.modlog_ref),
                            None
                        )
                        if modlog:
                            links_text += f"   └── 📊 {modlog.chat_name} (`{modlog.chat_id}`)\n"
                        else:
                            links_text += f"   └── ⚠️ Linked to unknown chat (`{moderated.modlog_ref}`)\n"
                    else:
            ...
            standalone_modlogs = [
                modlog for modlog in modlog_channels
    ...
    async def _show_config(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            config_text += f"• Provocation interval: {global_config.provocation_interval_hours} hours\n"
            ...
            config_text += "\n**Last Updated:**\n"
            config_text += f"• Config: {global_config.provenance.updated_at.strftime('%Y-%m-%d %H:%M')} UTC\n"
    ...
    async def _show_reports(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if not is_moderated:
                await update.message.reply_text(
                    "❌ Reports can only be viewed in moderated chats."
                )
                return
