...
def get_default_puzzles() -> list[Puzzle]:
    ...
    data = [
        (
            ...
            ["Cestui Que Vie trust", "Santa's Gift Fund", "Enid Blyton Trust", "Otter Conservation Fund"],
            0,
        ),
        (
            "What does Sovereign Pete claim DVLA 'registration' does to your car?",
            ["Transfers ownership to the state", "Doubles horsepower", "Makes it amphibious", "Erases speeding points"],
            0,
        ),
        (
        ...
        (
            ...
            ["TPR (Liverpool People's Resistance)", "Mensa Liverpool", "Knights of Columbus", "Rotary Club of Crosby"],
            0,
        ),
        (
        ...
        (
            ...
            ["It's 'Dog Latin' with no standing", "It's Morse code for taxes", "It's a naval distress flag", "It's an anagram for 'Crown'"],
            0,
        ),
        (
            "Fuel 'catalyst' grifters on Vobes's show claimed pellets worked in what?",
            ["Any engine: cars, bikes, even lawnmowers", "Only vintage carburetted cars", "Only diesel trucks over 3.5t", "Only hybrid vehicles"],
            0,
        ),
        (
            "Why do 'fuel-catalyst' claims about a galvanic reaction in petrol fail?",
            ["Petrol is an electrical insulator", "Diesel freezes at room temp", "Oxygen is a noble gas", "Water burns in engines"],
            0,
        ),
        (
            "Which trope explains why miracle devices 'never catch on'?",
            ["Grand suppression narrative", "Patent office backlog", "Quantum misalignment", "Astrological embargo"],
            0,
        ),
        (
            "Chemtrail believers misread which ordinary phenomenon?",
            ["Persistent contrails from jet exhaust", "Sun dogs at low angles", "Noctilucent clouds only at poles", "Sprites above thunderstorms"],
            0,
        ),
        (
        ...
        (
            ...
            ["Trying pipe tobacco", "Measuring chemtrails", "Building a Faraday cage", "Counting pyramids"],
            0,
        ),
        (
            "Which bogus fix did Vobes platform to 'save fuel'?",
            ["Drop-in tank pellets", "Aluminium foil on bonnet", "Stronger spark plugs only", "Inflatable spoiler"],
            0,
        ),
        (
        ...
        (
            ...
            ["Acceptance for Value", "In Perpetuity Forever", "Force Majeure Me", "Without Prejudice Pay"],
            0,
        ),
        (
            "What do sovereign-citizen influencers claim the term 'license' means?",
            ["Asking permission to use 'their' property", "Secret Crown lien", "Immunity from traffic laws", "Automatic diplomatic status"],
            0,
        ),
        (
            "Which scientific point dismantles 'fog full of nanobots' claims?",
            ["Weather and illness co-occur without causation", "Fog is dry ice residue", "Nanobots need 5G to swim", "Humidity kills viruses instantly"],
            0,
        ),
        (
            "What implausible authority did a TPR spokesman cite for global crimes?",
            ["Canada's Defence Minister", "Interpol's Pope", "UN Sheriff of Health", "MI5 Chief Pharmacist"],
            0,
        ),
        (
            "What courtroom step did Michael (with Liverpool TPR) claim he could refuse?",
            ["Entering a plea for the 'legal fiction'", "Sitting in the dock", "Standing for the judge", "Swearing on any book"],
            0,
        ),
        (
            "Which phrase do sovereigns misuse to claim 'government has no authority'?",
            ["Clearfield Doctrine", "Section 31 Immunity", "Rule in Shelley's Case", "Nemo Dat Doctrine"],
            0,
        ),
        (
        ...
        (
            ...
            ["Nothing scientific about 5G risk", "Guaranteed lethal rays", "Quantum weather hacks", "Ion engines in lampposts"],
            0,
        ),
        (
            "What mythical paperwork status do sovereigns seek for jurors?",
            ["'Sovereign men' not on electoral roll", "All jurors must be tradesmen", "Jury of internet subscribers", "Only unlicensed drivers allowed"],
            0,
        ),
        (
            "What everyday thing did Vobes compare to 'financial noclip mode'?",
            ["Signing bills to 'pay' them", "Using contactless twice", "Refunding a refund", "Tapping a lamppost"],
            0,
        ),
        (
        ...
        (
            ...
            ["An original wet-ink signature", "A proclamation of common law", "A gold seal proving nobility", "A chemtrail-resistant watermark"],
            0,
        ),
        (
        ...
        (
            ...
            ["A sovereign cannot be subject—so they invent loopholes", "Sovereigns get free petrol", "Sovereigns outrank traffic lights", "Sovereigns vote twice"],
            0,
        ),
        (
            "What sky claim does Ian Livingstone make?",
            ["He can spot 'real' vs 'fake' clouds", "He owns a private weather map", "He pilots anti-cloud drones", "He times rainbow durations"],
            0,
        ),
        (
            "Which absurd machine is invoked to 'manufacture clouds'?",
            ["A mainframe with an antenna", "A lawnmower on a kite", "A copper pyramid engine", "An inflatable Tesla coil"],
            0,
        ),
        (
            "If a 'mainframe' makes clouds, what happens to chemtrails logic?",
            ["It contradicts itself", "It becomes proven", "It needs bigger planes", "It requires 6G"],
            0,
        ),
        (
            "What basic property of petrol prevents any 'galvanic reaction' in a fuel tank?",
            ["No ions: current can't flow", "Wrong pH for sparks", "Too cold to oxidize", "Too viscous for voltage"],
            0,
        ),
        (
            "Which sales tactic do grifters use on Vobes's channel?",
            ["Anonymous testimonials and vague tech", "ISO lab certification", "Peer-reviewed trials", "Refund guarantees honored"],
            0,
        ),
        (
            "What role does Richard Vobes effectively play for product scammers?",
            ["Their marketing department", "Their compliance auditor", "Their chief engineer", "Their legal counsel"],
            0,
        ),
        (
            "What everyday agency do sovereigns insist is purely 'corporate'?",
            ["United Kingdom via DVLA", "NHS surgical wards", "BBC local radio", "National Trust properties"],
            0,
        ),
        (
            "What do sovereign-citizen influencers claim about UK driving test pass rates?",
            ["Only 49% can ever pass by design", "You pass automatically at 30", "Manual cars get bonus points", "Sunroof owners always fail"],
            0,
        ),
        (
        ...
        (
            ...
            ["Kick your door in without warrant", "Seize your goldfish", "Rewrite your MOT", "Downgrade your license"],
            0,
        ),
        (
            "Which fictional 'code' do sovereigns claim governs roads?",
            ["Corporate Highway Code contract", "Pirate Admiralty Roadbook", "Royal Coachman's Ledger", "Motor Magna Carta"],
            0,
        ),
        (
            "Why are 'fuel conspiracies' attractive to guests on Richard Vobes's channel?",
            ["They sell easy, magical fixes", "They require lab skill", "They need patents first", "They reduce emissions reliably"],
            0,
        ),
        (
            "What mundane explanation beats 'chemtrail' timing coincidences?",
            ["Weather + illness co-occur often", "Moon phase dictates trails", "Pilot's mood affects clouds", "Airport lunch menu changes"],
            0,
        ),
        (
            "Sovereign paperwork often seeks what escape?",
            ["Freedom from consequences", "Extra voting rights", "Secret tax refunds", "Airport lounge access"],
            0,
        ),
        (
            "What should skeptics do when hearing grand claims from conspiracy influencers?",
            ["Skepticism should also question grifters", "Questioning equals truth", "Belief beats data", "Intuition trumps physics"],
            0,
        ),
        (
            "The 'Crown has no standing' rant bundled what?",
            ["A grab-bag of misused international laws", "Precise case law citations", "Accurate treaty summaries", "Verified lab results"],
            0,
        ),
        (
        ...
        (
            ...
            ["They flatter believers as heroes", "They demand hard work", "They punish lying", "They hate attention"],
            0,
        ),
        (
            "What 'legal fiction' trick did Michael's group attempt?",
            ["Power of attorney games", "Embassy-as-home claim", "Common law ID badge", "Sovereign postage stamp"],
            0,
        ),
        (
            "In the fog panic, what measurement was requested?",
            ["Use a voltmeter on the fog", "Send a stool sample", "Weigh a cloud at noon", "Time raindrop speed"],
            0,
        ),
        (
        ...
        (
            ...
            ["Humidity/temperature at altitude", "Magnetic north drift", "Solar wind pressure", "Cosmic ray showers"],
            0,
        ),
        (
            "What does 'acceptance for value' promise believers?",
            ["Debt erasure by signature", "Cheaper mortgages", "Free road tax", "Unlimited childcare"],
            0,
        ),
        (
            "Which character archetype do scammers assume on Vobes's show?",
            ["Tortured, suppressed prophet", "Reluctant billionaire", "Accidental genius chef", "Undercover meteorologist"],
            0,
        ),
        (
            "The 'lawnmower test' mocked what claim?",
            ["Universal fuel gadget works anywhere", "Grass blocks airflow", "Two-stroke engines can't idle", "Electric mowers drink petrol"],
            0,
        ),
        (
            "When jurors are on the electoral roll, the sovereign says what?",
            ["They're 'subjects', not peers", "They are 'aliens' in law", "They owe maritime fees", "They must wear caps"],
            0,
        ),
        (
            "What do sovereign citizens claim about the ALL CAPS 'strawman' vs the living person?",
            ["ALL CAPS is a separate corporate entity", "Itals indicate maritime law applies", "Emojis indicate consent to contract", "Invisible ink proves sovereignty"],
            0,
        ),
        (
        ...
        (
            ...
            ["Confusion marketed as mystery", "Instant compliance", "Blank acceptance", "Shredding on sight"],
            0,
        ),
        (
        ...
        (
            ...
            ["Refined hydrocarbons lack free ions", "Diesel dissolves magnets", "Octane resists gravity", "Petrol emits cold heat"],
            0,
        ),
        (
            "Which line best sums up sovereign-citizen logic?",
            ["Invent loopholes to dodge reality", "Respect law to fix policy", "Use courts to clarify science", "Balance rights with duties"],
            0,
        ),
        (
            "Which UK doc is joked about as needing 'wet ink'?",
            ["Any 'warrant' they don't like", "Supermarket receipt", "Passport photo page", "Cinema ticket"],
            0,
        ),
        (
            "In spoof legalisms, what is 'spelling'?",
            ["'Witchcraft' that binds you", "A maritime tax trap", "A VAT for letters", "A railway bylaw"],
            0,
        ),
        (
            "What do sovereign-citizen narratives implicitly promise followers?",
            ["Escapism from consequences", "Expertise in explosives", "Soap-making metaphors", "Basement leases"],
            0,
        ),
        (
            "What cure-all status did Ardis ascribe to nicotine?",
            ["Preventative/curative for Alzheimer's", "Antibiotic for colds", "Antidote to chemtrails", "Vaccine booster"],
            0,
        ),
        (
            "Why are 'suppressed devices' stories convenient?",
            ["They justify selling unproven junk", "They demand peer review first", "They forbid online sales", "They cap profits"],
            0,
        ),
        (
            "What do believers forget about illness spikes and weather?",
            ["Correlation isn't causation", "Colds require 5G", "Fog produces viruses", "Rain sterilizes cities"],
            0,
        ),
        (
            "What do scam product demonstrations frequently avoid?",
            ["Transparent methodology and data", "Friendly audiences", "Dramatic music cues", "PowerPoint slides"],
            0,
        ),
        (
        ...
        (
            ...
            ["Strawman in ALL CAPS", "Ghost debtor in italics", "Maritime echo in brackets", "Shadow ID in bold"],
            0,
        ),
        (
        ...
        (
            ...
            ["'Embassy' houses immune to law", "'Cloud nation' passports", "'Sovereign tents' on roads", "'Sea lane' driveways"],
            0,
        ),
        (
        ...
        (
            ...
            ["It promises reward without effort", "It needs training", "It builds community", "It funds schools"],
            0,
        ),
        (
        ...
        (
            ...
            ["User error or misuse", "Nuclear fallout", "Tesla coils nearby", "X-ray beams in lampposts"],
            0,
        ),
        (
            "What UK paperwork do sovereigns demand to see on the spot?",
            ["Original wet-ink warrant", "MP's voting record", "Council's Wi-Fi password", "Judge's private notes"],
            0,
        ),
        (
            "Why is 'jury of sovereigns only' impossible?",
            ["It defies basic civic rules", "It requires DNA tests", "It needs royal assent", "It must be televised"],
            0,
        ),
        (
            "What product type is a red flag on Vobes's show?",
            ["Universal miracle fix", "Open-source software", "Workshop manuals", "Tyre pressure gauges"],
            0,
        ),
        (
            "Banaman's videos often begin with what?",
            ["Confusion about the obvious", "A legal citation spree", "Drone establishing shots", "A weather report"],
            0,
        ),
        (
        ...
        (
            ...
            ["You can't loophole reality", "Paper beats physics", "Ink outruns gravity", "Caps lock trumps law"],
            0,
        ),
        (
            "What do 'sovereign' court antics usually produce?",
            ["Self-sabotage and delays", "Instant acquittal", "Case dismissal with apology", "Jury standing ovations"],
            0,
        ),
        (
        ...
        (
            ...
            ["Atmospheric conditions differ", "Plane brand logos differ", "Pilot birthdays differ", "Fuel octane birthdays"],
            0,
        ),
        (
            "What kind of 'proof' do chemtrailers offer most?",
            ["Anecdotes and vibes", "Weather station logs", "Satellite humidity maps", "Peer-reviewed aerosol spectra"],
            0,
        ),
        (
            "Why are 'mainframe weather beams' mocked?",
            ["No mechanism or evidence", "They aim too high", "They cost £12.50", "They need moonlight"],
            0,
        ),
        (
            "What rhetorical move frames grifters as martyrs?",
            ["Cassandra/Galileo comparison", "Robin Hood swagger", "Sherlock deduction", "Bond villain monologue"],
            0,
        ),
        (
        ...
        (
            ...
            ["Transport via DVLA", "Housing via HM Land Reg", "Defence via MOD", "Culture via DCMS"],
            0,
        ),
        (
            "The show's view of 'question everything' is what?",
            ["Question grifters hardest", "Question only the state", "Question clouds exclusively", "Question never—just feel"],
            0,
        ),
        (
            "Why do chemtrail narratives appeal to believers?",
            ["They explain any weather as deliberate action", "They limit claims to rare events", "They are falsified by every cloud", "They require no airplanes"],
            0,
        ),
        (
            "What petty 'test' of respect do some influencers promote at dinner?",
            ["Stealing a chip at dinner", "Holding the TV remote", "Parking left of bins", "Counting teaspoons"],
            0,
        ),
        (
            "Why do 'magic signatures' fail in reality?",
            ["Contracts and debts require payment", "Pens lack legal ink", "Paper is maritime-only", "Numbers outrank names"],
            0,
        ),
        (
            "Why do simple miracle products appeal to conspiracy audiences?",
            ["Simple stories beat complex truths", "Labs are too noisy", "Pilots fly too low", "Meters beep too softly"],
            0,
        ),
        (
            "What's the comedic fate of many Vobes-endorsed gadgets?",
            ["No evidence, then silence", "Immediate regulation", "Open-source replication", "University adoption"],
            0,
        ),
        (
        ...
        (
            ...
            ["They allege the Crown has no standing", "They allege the Crown owns all juries", "They allege the Crown is a charity", "They allege the Crown flies the planes"],
            0,
        ),
        (
            "Sovereigns claim jurors on the roll are what?",
            ["State-owned 'enterprises'", "Royal witnesses", "Privateers at sea", "Unlettered clerks"],
            0,
        ),
        (
            "Why doesn't 'OBD miracle dongle' save petrol?",
            ["It can't change combustion chemistry", "OBD only powers USB", "It drains windshield fluid", "It reroutes exhaust"],
            0,
        ),
        (
            "What should you demand from grand claims by conspiracy promoters?",
            ["Demand mechanisms and data", "Cheer first, test later", "Buy now, think later", "Accept if suppressed"],
            0,
        ),
        (
        ...
        (
            ...
            ["Folksy platform for grifters", "Neutral expert debates", "Rigorous scientific trials", "Consumer watchdog reviews"],
            0,
        ),
        (
            "Why do 'cloud fake/real' claims fail?",
            ["Cloud physics doesn't care about vibes", "Clouds obey astrology", "Clouds require copper", "Clouds read blogs"],
            0,
        ),
        (
            "What hard lesson do sovereign-citizen tactics ignore?",
            ["No paperwork beats reality", "Shortcut if crowds cheer", "Always film the meter", "Trust every pellet"],
            0,
        ),
        (
            "Who ultimately pays for grifts promoted to conspiracy audiences?",
            ["The audience and their wallets", "The patent office", "Airline catering", "Town criers"],
            0,
        ),
        (
            "What classic fallacy powers the suppression myth?",
            ["Appeal to persecution", "Appeal to nature", "Appeal to novelty", "Appeal to authority"],
            0,
        ),
        (
            "What evidence standard should be applied to extraordinary claims?",
            ["Extraordinary claims need strong evidence", "Screenshots suffice", "Meters trump labs", "Humor trumps data"],
            0,
        ),
        (
            "Why is 'wet-ink' obsession mocked?",
            ["Modern warrants are digital records", "Ink activates maritime law", "Ink holds royal DNA", "Ink unlocks juries"],
            0,
        ),
        (
            "What's a practical way to evaluate 'too-good-to-be-true' products?",
            ["Be skeptical of magical fixes", "Film every cloud", "Refuse to register cars", "Shout at lampposts"],
            0,
        ),
        (
            "What two things do scammers fear most?",
            ["Transparency and replication", "Crowds and clapping", "Rhymes and riddles", "Maps and timetables"],
            0,
        ),
        (
            "Which habit helps avoid being scammed by conspiracy promoters?",
            ["Checking the boring details", "Skipping the footnotes", "Trusting the vibes", "Buying in bulk"],
            0,
        ),
        (
        ...
        (
            ...
            ["Freedom from paying and from blame", "Freedom to test hypotheses", "Freedom to hike hills", "Freedom to read papers"],
            0,
        ),
    ]
