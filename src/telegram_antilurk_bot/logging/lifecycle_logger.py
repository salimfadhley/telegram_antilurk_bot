...
class LifecycleLogger:
    ...
    async def log_provocation_created(
        self, provocation_id: int, chat_id: int, user_id: int, puzzle_id: str
    ) -> None:
        """Log provocation creation."""
        await self.provocation_logger.log_provocation_created(
            provocation_id=provocation_id, chat_id=chat_id, user_id=user_id, puzzle_id=puzzle_id
        )

    async def log_provocation_response(
        self, provocation_id: int, user_id: int, is_correct: bool
    ) -> None:
        """Log provocation response."""
        await self.provocation_logger.log_provocation_response(
            provocation_id=provocation_id, user_id=user_id, is_correct=is_correct
        )

    async def log_modlog_notification_sent(self, provocation_id: int, modlog_chat_id: int) -> None:
    ...
    async def log_kick_dismissed(
        self, provocation_id: int, admin_user_id: int, dismissal_reason: str | None = None
    ) -> None:
    ...
    async def get_complete_history(self, provocation_id: int) -> list[dict[str, Any]]:
        ...
        additional_events = [
            event for event in self._additional_events if event["provocation_id"] == provocation_id
        ]
    ...
    async def get_lifecycle_stats(self) -> dict[str, Any]:
        ...
        modlog_notifications = len(
            [e for e in all_additional_events if e["event"] == "modlog_notified"]
        )

        kick_confirmations = len(
            [e for e in all_additional_events if e["event"] == "kick_confirmed"]
        )

        kick_dismissals = len([e for e in all_additional_events if e["event"] == "kick_dismissed"])
